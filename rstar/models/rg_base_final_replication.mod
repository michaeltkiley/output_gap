...
stderr e_t2, .5, 0.0001, 1000,INV_GAMMA_PDF,.25,.5;
...
// datafile/nograph adjusted for this project's headless container (no
// LaTeX, no interactive plotting); mode_compute/mh_replic/mh_jscale/etc.
// left exactly as the author's original "base" (posterior-mode-only) spec.
estimation(order=1,optim=('MaxIter',1000,'Tolfun',1.0e-06),datafile='../data/observables_replication.csv',mode_compute=5,mode_file=rg_base_final_mode,first_obs=5,presample=4,lik_init=2,prefilter=1,mh_replic=0,mh_nblocks=2,mh_jscale=0.35,mh_drop=0.2,filtered_vars,smoothed_state_uncertainty,nograph) yU uU tr tg;
